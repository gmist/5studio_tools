...
func main() {
	fmt.Println("Экспорт товаров компании \"Город Игр\" в YML-файл для Я.Маркет")
	currentTime := time.Now().Format("2006-01-02-15-04-05")

	fromFile := os.Args[1:]

	if len(fromFile) == 0 {
		fmt.Println("Не указан файл со списком номеклатур для экспорта")
		fmt.Println("Использование: export_yml_yandex product_file.csv")
		return
	}
